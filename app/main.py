...
# ---- Minimal web UI at /ui (no extra deps) ----
UI_HTML = """
<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  <title>CV Score – Mini UI</title>
...
<body>
  <h1>CV Score <span class="pill">mini UI</span></h1>
  <p class="muted">Create a Job, add Candidates & CVs, then run matching and see the Top-5 with suggestions.</p>
  ...
  <div class="card">
    ...
    <label style="margin-top:12px;">CV Text</label>
...
async function createCandidate() {
  ...
  const payload = { external_ref: document.getElementById("cand_ref").value || null, anonymized: document.getElementById("cand_anon").checked || false };
...
async function getResults() {
  ...
  document.getElementById("results").innerHTML = `<table><thead><tr><th>Candidate</th><th>Score</th><th>Rank</th><th>Suggestions</th></tr></thead><tbody>${rows || "<tr><td colspan='4'>No results.</td></tr>"}</tbody></table>`;
}
</script>
</body>
</html>
