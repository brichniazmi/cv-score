...
# Default weights (editable per job)
WEIGHTS = {
    "req_skills": 0.35,
    "pref_skills": 0.15,
    "role_relevance": 0.15,
...
class Subscores:
    ...
    education: float = 0.0
    languages: float = 0.0
    continuity: float = 1.0  # neutral default

    def to_dict(self):
        return {
            k: getattr(self, k) for k in WEIGHTS.keys()
        }

ROLE_KEYWORDS = {
    # naive mapping; extend with taxonomy/embeddings later
...
def compute_subscores(jd: dict, cv_text: str) -> Tuple[Subscores, List[str]]:
    ...
    # crude role relevance from title keywords
    ...
    # achievement density: count numbers/% in bullet-like lines
    ...
    # education / languages placeholders (set via parsed_json in future)
    subs.education = 0.5
    subs.languages = 0.5

    # Hard blocker example: if job lists a mandatory cert
